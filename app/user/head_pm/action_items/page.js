...
export default function HeadPMActionItems() {
  ...
  // When title filter is active: flat table showing everyone with that title
  ...
  const renderPersonGrouped = () => {
    ...
    if (netIds.length === 0) return (
      <div className={styles.emptyState}>
        ...
        {statusFilter === "open" ? "No pending items" : "No items here yet"}
      </div>
    );
            ...
            {form.target_type === "individual" && (
              <div className={styles.formGroup}>
                ...
                <input
                  ...
                  placeholder="jdoe2"
                  list="hpm-ai-users-list"
                />
                <datalist id="hpm-ai-users-list">
                  {[...pms, ...students].map((u) => (
